...
if "SPOTIFY_CLIENT_ID" not in st.secrets or "SPOTIFY_CLIENT_SECRET" not in st.secrets:
    st.error("Deploy前に Streamlit Secrets に SPOTIFY_CLIENT_ID と SPOTIFY_CLIENT_SECRET を設定してください。")
    st.stop()
...
def audio_features_for_ids(ids):
    ...
    for i in range(0, len(ids), 50):
        ...
        try:
            ...
            # None が返る場合もあるのでフィルタ
            feats.extend([f if f else {"id": batch[j], "danceability":0,"energy":0,"valence":0,"tempo":0,"acousticness":0,"instrumentalness":0,"liveness":0} for j,f in enumerate(res)])
        except spotipy.SpotifyException as e:
            st.warning(f"Audio features fetch failed for batch {batch[:5]}...: {e}")
            # 失敗したバッチはゼロ埋めで続行
            feats.extend([{"id":x,"danceability":0,"energy":0,"valence":0,"tempo":0,"acousticness":0,"instrumentalness":0,"liveness":0} for x in batch])
    return feats
...
if st.button("おすすめを探す"):
    with st.spinner("検索中..."):
        ...
        if not ida or not idb:
            st.error("AAA/BBB の曲が見つかりません。入力を確認してください。")
            st.stop()
        id_artist = get_artist_id(artist_bb)
        if not id_artist:
            st.error("BB のアーティストが見つかりません。")
            st.stop()

        ids_meta = get_artist_track_ids(id_artist, limit=300)
        if not ids_meta:
            st.error("BB の曲が見つかりません。")
            st.stop()
        ...
        if not t_feats or any(f is None for f in t_feats):
            st.error("AAA/BBB の音響特徴が取得できません")
            st.stop()
